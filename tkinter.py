...
class Root(Tk):
    def __init__(self):
        ...
        self.plate_characters = ttk.Label(self.labelFrame)
        self.plate_characters.grid(column=0, row=3, columnspan=3)
    ...
    def fileDialog(self):
        filename = filedialog.askopenfilename(initialdir="C:/Năm 3/", title="Select A File")
    ...
    def image_labels(self, image):
        ...
        if len(region_pass) == 1:
            self.plate_crops(self.image, region_pass[0])
        else:
            pass
        # self.license_plate_display(img_displayed)

    def plate_crops(self, image, cordinate):
        cropped_image = image.crop(cordinate)
        self.license_plate_display(cropped_image)
        self.characters_recognize(cropped_image)
        cropped_image.save('license_plate.png')

    def characters_recognize(self, image):
        custom_config = r'--oem 3 --psm 6'
        print(tes.image_to_string(image, config=custom_config))
        self.plate_characters.configure(text=tes.image_to_string(image, config=custom_config), font=("Arial", 25))
